...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
  <div class="flex items-center justify-center w-full min-h-screen">
      ...
      <!-- Right form section -->
      <div class="w-full md:w-1/2 p-10" method="POST" action="{{ route('login.store') }}">
        <h2 class="text-3xl font-semibold text-gray-800 mb-8">Welcome to Cookeria</h2>
        <form class="space-y-6" method="POST" action="{{ route('login.store') }}">
          ...
          <div>
            ...
            <input
              ...
              required
              class="w-full border border-gray-300 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                value="{{ old('email') }}"
            />
